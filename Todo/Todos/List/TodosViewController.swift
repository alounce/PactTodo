...
class TodosViewController: UIViewController {
    ...
    func loadData() {
        viewModel.download { [weak self] result in
            ...
            case .failure(let error):
                print("Error: \(error)")
                SCLAlertView()
                    .showError("Cannot load data", subTitle: error.localizedDescription)
            }
        }
    }
    ...
    func delete(_ todo: Todo) {
        ...
        alertView.addButton("Delete") {
            self.viewModel.delete(todo) { [weak self] result in
...
extension TodosViewController: TodoTableViewCellDelegate {
    func todoCell(_ cell: TodoTableViewCell, didChangeCompletion completed: Bool) {
        ...
        viewModel.save(todo) { [weak self] result in
            switch result {
            case .success(_):
                if let ip = self?.tableView.indexPath(for: cell) {
                    self?.tableView.reloadRows(at: [ip], with: UITableView.RowAnimation.automatic)
                }
            case .failure(let error):
                print(error.localizedDescription)
